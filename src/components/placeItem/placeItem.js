...
class PlaceItem extends Component{
    ...
    render(){
        ...
        return(
            <a href="#" onClick={() => this.handleSelect(place)} className="list-group-item list-group-item-action flex-column align-items-start mb-1">
                <div className="d-flex w-100 justify-content-between">
                    <h5 className="mb-1">{place.name}</h5>
                    <small>{distance} miles</small>
                </div>
                <p>{place.vicinity}</p>
                <span className={this.isOpen(place) ? 'badge badge-primary' : 'badge badge-light'}>{this.isOpen(place) ? 'Open' : 'Closed'}</span>
